...
这个模板用于新建一个公开仓库，让公开仓库通过 GitHub Actions 拉取当前私有 `Legado-Tauri` 指定 commit，再免费构建并发布 Windows / Android / macOS / Harmony 压缩包。
...
- `scripts/create-protected-archives.ps1`
  - 把 `.exe` / `.apk` / `.dmg` / `.hap` 打成带密码的压缩包，再上传到公开仓库 Release。
...
- 私库需配置：
  ...
  - Actions Variable: `PUBLIC_BUILD_TARGETS=windows,android,macos`

要自动加入 macOS / Harmony 时，再把：

- `PUBLIC_BUILD_TARGETS`
  - 改成 `windows,android,macos,harmony`
  - 或直接改成 `all`
...
- 如果公开仓库已经建好，私库模板更新后还需要把 `scripts/public-build-repo-template/.github/workflows/public-build.yml`、`scripts/public-build-repo-template/scripts/prepare-public-build-artifacts.mjs`、`scripts/public-build-repo-template/scripts/create-protected-archives.ps1`、`scripts/public-build-repo-template/scripts/build-release-notes.ps1` 以及 `scripts/public-build-repo-template/scripts/send-qq-message.py` 同步到公开仓库实际文件；只改私库模板不会自动修好线上 workflow。
...
- Windows、Android、macOS、Harmony 分开构建，最后统一收集并打密码压缩包发布到公开仓库 Release。
...
- `both` 仍表示 `windows + android`，`all` 表示 `windows + android + macos + harmony`，避免在未准备好 Harmony 工具链时误触发。
